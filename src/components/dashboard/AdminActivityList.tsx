// src/components/dashboard/AdminActivityList.tsx
...
function formatAgo(v: any): string {
  const t = tsToMs(v);
  if (!t) return "now";
...
function isActive(lastSeen: any): boolean {
...
export default function AdminActivityList() {
  ...
  async function load() {
    ...
    try {
      ...
      const mapped: Row[] = arr.map((s: any) => {
        ...
        return { deviceId, admin, lastSeen, active: isActive(lastSeen) };
      });
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 min-w-0">
                <span
                  ...
                  aria-hidden
                />
                <div className="text-sm font-semibold text-gray-800 truncate">{r.deviceId}</div>
              </div>
              <div className="text-[11px] text-gray-500 truncate">admin: {r.admin}</div>
            </div>
  ...
  return (
    <div className="bg-white rounded-2xl border">
      ...
      {/* ✅ FULL-WIDTH Manage button (cannot hide) */}
      <div className="px-4 pt-3">
        <Link
          to="/sessions"
          className="w-full h-10 inline-flex items-center justify-center rounded-xl border bg-white text-sm font-semibold text-gray-700 hover:bg-gray-50 active:scale-[0.99]"
        >
          Manage Sessions
        </Link>
      </div>
